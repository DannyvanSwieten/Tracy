...
use crate::math::{Mat4, Point, Real, Vec3};
...
use cgmath::{perspective, vec3, Rad, Transform};
...
impl Renderer {
    pub fn new(device: Rc<DeviceContext>, width: u32, height: u32) -> Self {
        ...
        let mut result = Self {
            ...
            camera_position: Point::new(1., 1., 5.),
            camera_target: Point::new(0.0, 0.0, 0.0),
            current_batch: 0,
        };
    ...
    fn build_camera_buffer(&self, device: Rc<DeviceContext>) -> BufferResource {
        let view_matrix =
            Mat4::look_at_rh(self.camera_position, self.camera_target, vec3(0., 1., 0.));
        let _view_inverse = view_matrix.inverse_transform().unwrap();
        ...
        let _projection_inverse = projection_matrix.inverse_transform().unwrap();
